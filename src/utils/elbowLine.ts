  ...
  if (pr < q.x || q.x < pl) {
    ...
    if (pb < qt) {
      return [
        { x: p.x, y: (pb + qt) / 2 },
        { x: ql - margin, y: (pb + qt) / 2 },
        { x: ql - margin, y: qb + margin },
        { x: q.x, y: qb + margin },
      ];
    } else if (qb < pt) {
      return [
        { x: p.x, y: pb + margin },
        { x: ql - margin, y: pb + margin },
        { x: ql - margin, y: (qb + pt) / 2 },
        { x: q.x, y: (qb + pt) / 2 },
      ];
    } else {
      return [
  ...
  if (pb < q.y || q.y < pt) {
    ...
    if (pr < ql) {
      return [
        { x: (pr + ql) / 2, y: p.y },
        { x: (pr + ql) / 2, y: pb + margin },
        { x: qr + margin, y: pb + margin },
        { x: qr + margin, y: q.y },
      ];
    } else if (qr < pl) {
      return [
        { x: pr + margin, y: p.y },
        { x: pr + margin, y: qb + margin },
        { x: (qr + pl) / 2, y: qb + margin },
        { x: (qr + pl) / 2, y: q.y },
      ];
    } else {
