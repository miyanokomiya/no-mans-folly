...
function getLineRelatedLayoutPatch(
  ...
  const step = (sc: ShapeComposite, currentPatch: { [id: string]: Partial<Shape> }) => {
    ...
    latestPatch = mergePatchForLineVertices(sc, latestPatch, result.patch);
 ...
 */
function mergePatchForLineVertices(
  sc: ShapeComposite,
  latestPatch: Record<string, Partial<Shape>>,
  stepPatch: Record<string, Partial<Shape>>,
): Record<string, Partial<Shape>> {
  const mergedLatestPatch = mergeMap(latestPatch, stepPatch);
  mapEach(stepPatch, (patch, id) => {
