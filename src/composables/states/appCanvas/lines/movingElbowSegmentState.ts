      ...
      switch (event.type) {
        case "pointermove": {
          snappingResult = event.data.ctrl
            ? undefined
            : shapeSnapping.test(
                moveRect(getOuterRectangle([targetSegment]), sub(event.data.current, event.data.start)),
              );
          const p = snappingResult ? add(event.data.current, snappingResult.diff) : event.data.current;
